...
;; malloc : size roots roots -> address
...
(define (malloc n roots1 roots2)
  ...
  (if (heap-ref 1)
      (cond 
        [(>= (+ address n) (heap-size)) (collect-garbage roots1 roots2)
                                        (define address (heap-ref 0))
        ...
        [else (heap-set! 0 (+ address n))
              address])
      (cond 
        [(>= (+ address n) (+ 4 (/ (- (heap-size) 4) 2))) (collect-garbage roots1 roots2)
                                                          (define address (heap-ref 0))
                                                          (heap-set! 0 (+ address n))
                                                          (if (heap-ref 1)
                                                              (when (>= (heap-ref 0) (heap-size))
  ...
  (define (loop i)
    (when (< i (heap-ref 0))
             (case (heap-ref i)
               ...
               [else (error 'validate-heap "found an unknown tag @ ~a" i)])))
    (loop (if (heap-ref 1)
...
(define (gc:alloc-flat value)
  (define address (malloc 2 false false))
...
(define (gc:closure code-ptr free-vars)
  (define address (malloc (+ 3 (length free-vars)) free-vars false))
